document.addEventListener("DOMContentLoaded", function () {
    ...
    let background = document.querySelector(".advant-background");

    // Store the index of the last hovered card
    let lastHoveredCardIndex = localStorage.getItem("lastHoveredCardIndex") || 0;

    // Set the background to the last hovered card by default
    let cardRect = cards[lastHoveredCardIndex].getBoundingClientRect();
    let x = cardRect.left + window.scrollX + cardRect.width / 2;
    let y = cardRect.top + window.scrollY + cardRect.height / 2;
